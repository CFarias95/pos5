              ...
              <div class="row">
                ...
                <div class="col-sm-12 col-md-3 col-lg-3">
                  ...
                  <div class="form-group" v-if="form.machine_id">
                    <el-tag type="danger" effect="dark"> Min: {{ min_force }} </el-tag>

                    <el-tag type="success" effect="dark"> Max: {{ max_force }} </el-tag>
                  </div>
                </div>
                ...
                <div class="col-sm-12 col-md-3 col-lg-3">
                  <div :class="{ 'has-danger': errors.agreed }" class="form-group">
                    <label class="control-label"> Conformes </label>
                    <el-input-number
                      v-model="form.agreed"
                      :controls="false"
                      :min="0"
                ...
                <div class="col-12 mt-3">
                  <div class="form-group">
                    <label class="control-label"> Mezcla </label>
                  </div>
                </div>
                    ...
                    <div class="col-6">
                      ...
                      >
                        ...
                        <small
                          ...
                          class="form-control-feedback"
                          v-text="errors.mix_date_start[0]"
                        ></small>
                      </div>
                    </div>
                    <div class="col-6">
                      ...
                      >
                        ...
                        <small
                          ...
                          class="form-control-feedback"
                          v-text="errors.mix_time_start[0]"
                        ></small>
                      </div>
                    </div>
                  </div>
...
export default {
  ...
  methods: {
    ...
    warehouse_stock(index) {
      const selectedWarehouseId = this.supplies[index].warehouse_id;
      ...
      const itemWarehouse = this.item_warehouses.find(
        (iw) =>
          iw.warehouse_id === selectedWarehouseId &&
          iw.item_id === supply.individual_item_id
      );

      if (itemWarehouse) {
        supply.stock = itemWarehouse.stock;
        supply.difference = supply.stock - supply.quantityD;
      } else {
        supply.stock = 0;
        supply.difference = supply.stock - supply.quantityD;
      }

      if (supply.difference < 0) {
        this.$message.error("Tiene productos sin stock suficiente en ese almacen!");
      }

      // Actualiza solo el suministro en el índice dado
      this.supplies.splice(index, 1, supply);

      // Comprueba si existe alguna diferencia negativa después de la actualización
      this.checkNegativeDifferences();
    },

    checkNegativeDifferences() {
      this.supply_difference = this.supplies.some((supply) => supply.difference < 0);
    },
