...
class Tenantreportcomprasbaseimpuestos extends Migration
{
    ...
    public function up()
    {
        $sql_create = "CREATE PROCEDURE `SP_ComprasBaseImpuestos`(
	IN `date_start` DATE,
	IN `date_end` DATE,
	IN `supplier_id` INT,
	IN `doc_type_id` INT
...
BEGIN
                ...
                FROM purchases AS a INNER JOIN cat_purchase_document_types2 AS  b ON a.document_type_id = b.idtYpe
                ...
                WHERE a.date_of_issue>=date_start AND a.date_of_issue<=date_end
					 AND (supplier_id = d.id OR 0=supplier_id)
					 AND (doc_type_id = a.document_type_intern OR 0 = doc_type_id);
            END";
