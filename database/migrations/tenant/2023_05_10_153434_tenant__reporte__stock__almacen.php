...
class TenantReporteStockAlmacen extends Migration
{
    ...
    public function up()
    {
        ...
        BEGIN
                            ...
                            dc.name AS departamento , cc.name AS seccion , sc.name as categoria, fc.name as familia, d.name Marca , b.unit_type_id Unidadmedida, CASE WHEN b.purchase_mean_cost IS NULL THEN 0 ELSE
        ...
        @num := 1 + LENGTH(category_id_array) - LENGTH(REPLACE(category_id_array, '','', ''''))              AS num,
        IF(@num > 1,LTRIM(RTRIM(REPLACE(REPLACE(SUBSTRING_INDEX(category_id_array, '','', 1),''['',''''),'']'',''''))) , NULL) as coddepartamento ,
        IF(@num > 1, LTRIM(RTRIM(REPLACE(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(category_id_array, '','', 2),'','',-1),''['',''''),'']'',''''))), NULL)  codcategoria,
        IF(@num > 2,LTRIM(RTRIM(REPLACE(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(category_id_array, '','', 3),'','',-1),''['',''''),'']'',''''))), NULL) as codseccion,
        IF(@num > 3,LTRIM(RTRIM(REPLACE(REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(category_id_array, '','', 4),'','',-1),''['',''''),'']'',''''))), NULL)  codfamilia
        from items as a
        WHERE category_id_array IS NOT NULL
