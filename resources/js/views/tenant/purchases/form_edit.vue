        ...
        <div class="form-body">
          ...
          <div class="row" v-if="form.items.length > 0">
                ...
                <table class="table">
                  ...
                  <tbody>
                    <tr v-for="(row, index) in form.items" :key="index">
                      ...
                      <td class="text-right">
                        {{ currency_type.symbol }} {{ row.total_discount }}
                      </td>
                      <td class="text-right">
                        {{ currency_type.symbol }} {{ row.total_charge }}
                      </td>
                      <td class="text-right">
            ...
            <div class="col-md-6">
              ...
              <p v-if="form.total_exportation > 0" class="text-right">
                OP.EXPORTACIÓN: {{ currency_type.symbol }} {{ form.total_exportation }}
              </p>
              <p v-if="form.total_free > 0" class="text-right">
                OP.GRATUITAS: {{ currency_type.symbol }} {{ form.total_free }}
              </p>
              <p v-if="form.total_unaffected > 0" class="text-right">
                SUBTOTAL 0%: {{ currency_type.symbol }} {{ form.total_unaffected }}
              </p>
              <p v-if="form.total_exonerated > 0" class="text-right">
                OP.EXONERADAS: {{ currency_type.symbol }} {{ form.total_exonerated }}
              </p>
              <p v-if="form.total_taxed > 0" class="text-right">
                SUBTOTAL 12%: {{ currency_type.symbol }} {{ form.total_taxed }}
              </p>
              ...
              <p v-if="form.total_ret > 0" class="text-right">
                RETENIDO: {{ currency_type.symbol }} {{ form.total_ret }}
              </p>

              <p v-if="form.total_discount > 0" class="text-right">
              ...
              <template v-if="is_perception_agent">
                ...
                <h3 v-if="form.total > 0 && !hide_button" class="text-right">
...
export default {
  ...
  methods: {
    ...
    getPayments(payments) {
      payments.forEach((it) => {
        it.payment_destination_id =
          it.global_payment.destination_type == "App\Models\Tenant\Cash"
            ? "cash"
    ...
    addAdvancesCustomer() {
      this.$http.get(`/documents/advance/${this.form.supplier_id}`).then((response) => {
    ...
    changeDocumentType2() {
      var document = _.find(this.document_types2, {
        idType: this.form.document_type_intern,
      });
      this.form.document_type_id = document.DocumentTypeID;
      this.codSustentos = _.filter(this.codSustentos_all, {
        idTipoComprobante: this.form.document_type_id,
      });
    ...
    addRetenciones() {
      this.form.ret = [];
      let total =
        this.form.total_taxed + this.form.total_unaffected + this.form.total_igv;
      let retenido = 0;

      this.retencionesActuales.forEach((row) => {
        let retencionLocal = {};
        retencionLocal.valor = parseFloat(row.valorRet);
        const retIvaDesc = _.find(this.retention_types_income, {
          code: row.codRetencion,
        });
        if (retIvaDesc) {
    ...
    calculateTotal() {
      ...
      this.form.items.forEach((row) => {
        ...
        if (
          ["10", "11", "12", "20", "30", "40"].indexOf(row.affectation_igv_type_id) < 0
        ) {
