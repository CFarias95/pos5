<template>
    <div class="card mb-0 pt-2 pt-md-0">
                                    ...
                                    <div class="form-group col-6 col-md-2">
                                        ...
                                        <el-select v-model="form.seller_id" clearable>
                                            <el-option v-for="sel in sellers" :key="sel.id" :value="sel.id"
                                                :label="sel.name">{{ sel.name }}</el-option>
                                        </el-select>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-lg-12 col-md-6 d-flex align-items-end">
                                <div class="form-group">
                                    <button type="button" class="btn waves-effect waves-light btn-primary"
                                        @click="clickAddItem">+ Agregar Producto</button>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <p class="text-right" v-if="form.total_free > 0">OP.GRATUITAS: {{ currency_type.symbol }} {{
                                    form.total_free }}</p>
                                <p class="text-right" v-if="form.total_unaffected > 0">SUBTOTAL 0%: {{ currency_type.symbol
        ...
        <quotation-form-item :showDialog.sync="showDialogAddItem" :currency-type-id-active="form.currency_type_id"
            :exchange-rate-sale="form.exchange_rate_sale" :typeUser="typeUser" :recordItem="recordItem"
            :configuration="config" :customer-id="form.customer_id" :percentage-igv="percentage_igv"
            :currency-types="currency_types" :show-option-change-currency="true" @add="addRow"></quotation-form-item>
    ...
    methods: {
        ...
        setTotalDefaultPayment() {
            ...
            if (this.form.payments.length > 0) {
                ...
                console.log('mensaje',this.form.payments)*/
                let payment_count = this.form.payments.length
                let total = this.form.total
                let payment = 0
                let amount = _.round(total / payment_count, 6)
