...
class ItemController extends Controller
{
    ...
    public function getCategoriesTree()
    {
        ...
        return $categories->transform(function ($row) {
            $categorien1 = Category::where('parent_id', $row->id)->where('parent_2_id', null)->get();
            if ($categorien1 && $categorien1->count() > 0) {
                return [
                    'value' => $row->id,
                    'label' => $row->name,
                    'children' => $categorien1->transform(function ($row1) {
                        $categorien2 = Category::where('parent_2_id', $row1->id)->where('parent_3_id', null)->get();
                        if ($categorien2 && $categorien2->count() > 0) {
                            return [
                                'value' => $row1->id,
                                'label' => $row1->name,
                                'children' => $categorien2->transform(function ($row2) {
                                    $categorien3 = Category::where('parent_3_id', $row2->id)->get();
                                    if ($categorien3 && $categorien3->count() > 0) {
                                        return [
                                            'value' => $row2->id,
                                            'label' => $row2->name,
                                            'children' => $categorien3->transform(function ($row3) {
                                                return [
                                                    'value' => $row3->id,
                                                    'label' => $row3->name,
                                                ];
