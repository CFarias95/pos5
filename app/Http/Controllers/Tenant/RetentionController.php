    ...
    public function import(Request $request)
    {
        try {
            ...
            if (count($dataArray) > 1) {
                ...
                if(str_contains($dataArray[0], 'RAZON_SOCIAL_EMISOR')){
                    foreach ($dataArray as $ret) {
                        ...
                        try {
                            ...
                            $ret = explode("\t", $ret);
                            if ($ret['1'] != "RAZON_SOCIAL_EMISOR") {

                                if (substr($ret['4'], 8, 2) != '07') {
                                    return;
                                }
                                ...
                                if ($comporbante > 0) {
                                    ...
                                    Log::error("No se encontro la retencion : " . $ret['4']);
                                    $totalError += 1;
                                }
                            }
                        } catch (Exception $ex) {
                ...
                if(str_contains($dataArray[0], 'SERIE_COMPROBANTE')){
                    foreach ($dataArray as $ret) {
                        $ret = str_replace("\t\t\t\t","\t",$ret);
                        $totalDocumentos += 1;
                        $number = (Retention::get())->count() + 1;
