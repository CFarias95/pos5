...
class DispatchesSriController extends Controller
{
    ...
    // Recopila todos los datos necesarion para formar la guia de remision
    private function prepareDocument($id){

        $dispatch = Dispatch::findOrFail($id);
        $dispatchD = DispatchItem::where('dispatch_id', $id)->get();
        ...
        if($dispatch->count() > 0  && $dispatchD->count() > 0){
            ...
            $guia = [
                ...
                'codDoc' => '06',
                'establecimiento' => $establecimiento->code,
                ...
                'contribuyenteEspecial' => (isset($this->company->contribuyente_especial_num))?$this->company->contribuyente_especial_num:'',
                'fechaIniTransporte'=> $dispatch->date_of_issue->format('d/m/Y'),
                'fechaFinTransporte'=> $dispatch->date_of_shipping->format('d/m/Y'),
                'placa' => $dispatch->license_plate,
                'identificacionDestinatario' => $dispatch->customer->number,
                'razonSocialDestinatario' => $dispatch->customer->name,
    ...
    //validamos el docuemnto en el SRI los que ya fueron enviados
